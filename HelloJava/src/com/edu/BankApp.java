...
//	  : 2.예금(계좌번호,예금액) > 최고예금금액 10만원. 
//	  : 3.출금(계좌번호,출금액) > 잔고보다 큰금액은 출금못하도록
//	  : 4.잔액조회(번호)
//    : 5.종료
...
//		System.out.println("예금기능");
		...
		for(int i=0; i<banks.length;i++) {
			if(banks[i] !=null && banks[i].getAccNo().equals(ano)) { //계좌번호가 있을경우
				//계좌번호가 조회되었을때
				checkCnt = 1; 
				int currAmt = banks[i].getMoney();
				if(currAmt+amt > 100000) {
					// 예금액이 10만원이 초과하지 못하도록.
					checkCnt = 2;
					break;
				}
