...
public class TextileOracle extends DAO implements TexIf {
	...
	@Override
	public List<TexTile> modlist() {
		conn = getConnect();
		List<TexTile> t1 = new ArrayList<TexTile>();
		String sql = "select*\r\n"
				+ "from textile\r\n"
				+ "order by order_nu";
		try {
			...
			rs = psmt.executeQuery();
			while (rs.next()) {
				TexTile t2 = new TexTile();
				t2.setTexnu(rs.getInt("order_nu"));
				t2.setTexna(rs.getString("tex_na"));
				t2.setTexco(rs.getString("tex_co"));
				t2.setTexam(rs.getInt("tex_md"));
				
				t2.setDate(rs.getString("tex_now"));
				t1.add(t2);
			}
		} catch (SQLException e) {
			e.printStackTrace();
